...
<hr></hr>  <form>
<h4><center>PURCHASE-----<input type=textbox></center></h4>
<hr></hr>  </form>
...
while($row=mysqli_fetch_array($result1)){
		...
		$sql1= "SELECT sum(igst)+SUM(cgst)+sum(cgst) from purchase";
$result1=$conn->query($sql1);
while($row=mysqli_fetch_array($result1)){
...
$result=$conn->query($sql);
while($row=mysqli_fetch_array($result)){
		echo "GST Sales:  ".$row['sum(cgst)+sum(igst)+sum(cgst)'];
		echo "<br>";
}
$sql= "SELECT sum(cgst)+sum(igst)+sum(cgst) from purchase";
$result=$conn->query($sql);
while($row=mysqli_fetch_array($result)){
		echo "GST Purchase:  ".$row['sum(cgst)+sum(igst)+sum(cgst)'];
		echo "<br>";
}
