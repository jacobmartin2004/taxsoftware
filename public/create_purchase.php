...
<head>
    ...
    <style>
        ...
        .invoice-box { max-width: 800px; margin: 24px auto; padding: 24px; background: #fff; border-radius: 10px; box-shadow: 0 0 15px rgba(0,0,0,0.1); }
...
<nav class="top-nav">
    ...
    <div class="nav-links">
        ...
        <a href="create_purchase.php" class="active"><i class="bi bi-cart-plus"></i> Purchase</a>
    ...
    <form method="POST" action="../src/save_invoice.php">
        ...
        <div class="row mb-3">
            <div class="col-md-4">
                ...
                <input type="number" class="form-control" id="inp_taxable" step="0.01" min="0" onchange="recalculate()" onkeyup="recalculate()" required>
            </div>
            <div class="col-md-4" id="cgst_col" style="display:none;">
                <label class="form-label">CGST (9%):</label>
                <input type="text" class="form-control" id="disp_cgst" readonly>
            </div>
            <div class="col-md-4" id="sgst_col" style="display:none;">
                <label class="form-label">SGST (9%):</label>
                <input type="text" class="form-control" id="disp_sgst" readonly>
            </div>
            <div class="col-md-4" id="igst_col" style="display:none;">
                <label class="form-label">IGST (18%):</label>
                <input type="text" class="form-control" id="disp_igst" readonly>
            </div>
        </div>
...
function recalculate() {
    var taxable = parseFloat($('#inp_taxable').val()) || 0;
    ...
    $('#disp_cgst').val(cgst.toFixed(2));
    $('#disp_sgst').val(sgst.toFixed(2));
    $('#disp_igst').val(igst.toFixed(2));
    $('#show_taxable').text(taxable.toFixed(2));
